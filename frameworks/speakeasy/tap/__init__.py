...
class SpeakEasy_TAP:
    ...
    def infer(self, data, save_dir):
        ...
        # 1. GENERATE SUB-QUERIES #
        ###########################
        print("Step 1: Generate subqueries")
        save_path = os.path.join(save_dir, "subqueries.json")

        if not os.path.exists(save_path):
            # Generate English sub-queries and get outputs
            ...
            subqueries = {
                query: {subquery: "" for subquery in cleaned_output}
            ...
            subqueries = json.load(open(save_path, "r"))

        #########################
        # 2. GET TAP TEST CASES #
        #########################
        print("Step 2: Run TAP for subqueries")
        save_path = os.path.join(save_dir, "tap_test_cases.json")

        if not os.path.exists(save_path):
            from baseline.tap import Baseline_TAP
            TAP_attacker = Baseline_TAP(model=self.model)
            tap_test_cases = {}

            assert len(subqueries) == len(data), "Length mismatch between subqueries and data."

            for subqueries_dict, query_dict in tqdm(zip(subqueries.values(), data), 
                                                    desc="Generating TAP test cases", 
                                                    total=len(subqueries)):
                query = query_dict["query"]
                tap_test_cases[query] = {}

                for subquery in tqdm(list(subqueries_dict), desc=f"Processing {query}", leave=False):
                    tap_test_case = TAP_attacker.generate_single_test_case(subquery, query_dict["target"])
                    tap_test_cases[query][subquery] = tap_test_case
            ...
            tap_test_cases = json.load(open(save_path, "r"))

        #########################
        # 3. GET TAP TEST CASES #
        #########################
        print("Step 3: Translate TAP test cases into predefined languages")
        save_path = os.path.join(save_dir, "translated_subqueries.json")
        ...
        if not os.path.exists(save_path):
            translated_subqueries = {
                query: {
                    sub_query: {
                        lang: (
                            f"{tap_case} Keep your response within 120 words."
                            if lang == "en"
                            else self.translator.translate_multilingual(
                                f"{tap_case} Keep your response within 120 words.",
                                source="en", 
                                target=lang
                            )
                    ...
                    for sub_query, tap_case in sub_query_dict.items()
                }
                for query, sub_query_dict in tap_test_cases.items()
            }
        ...
        ###########################
        print("Step 4: Answer multilingual subqueries")
        ...
        # 5. COMBINE RESPONSES #
        ########################
        print("Step 5: Select and combine best queries")
        save_path = os.path.join(save_dir, "responses_with_reward.json")

        # Score each response
        if not os.path.exists(save_path):
            # Filter responses that are not refusals
            filtered_responses_en = {
                query: {
                    sub_query: {
                        lang: response
                        for lang, response in responses.items()
                        if not contains_refusal_words(response)
                    }
                    for sub_query, responses in subqueries.items()
                    if any(
                        not contains_refusal_words(response)
                        for response in responses.values()
                    )
                }
                for query, subqueries in responses_en.items()
                if any(
                    any(
                        not contains_refusal_words(response)
                        for response in responses.values()
            ...
            # Apply reward models to obtain a score for each response
            resp_select_model = get_model(self.config.resp_select_model, self.device)
            responses_with_reward = resp_select_model.get_reward(filtered_responses_en)
